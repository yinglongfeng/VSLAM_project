...
void VisualOdometry::generateDisparityMap(const cv::Mat image_left, const cv::Mat image_right) {
  ...
  cv::Ptr<cv::stereo::StereoBinarySGBM> sgbm =
      cv::stereo::StereoBinarySGBM::create(min_disparity, number_of_disparities, kernel_size);
...
//    std::cout<<"fx is : "<<fx<<" fy is : "<<fy<<" cx is : "<<cx<<" cy is : "<<cy<<std::endl;
  ...
  for (int i = 0; i < currFrame2DPoints.size(); ++i) {
    ...
    if (disparity < 0.1) {
      ...
      disparity = (neighborsDisparity[0] + neighborsDisparity[1] + neighborsDisparity[2]
          + neighborsDisparity[3] + neighborsDisparity[4])/4;
    }
    if (disparity < 0.1) {
      disparity = minValue;
    }
...
//    cv::Mat dist_coeffs = cv::Mat::zeros(4,1,cv::DataType<double>::type); // Assuming no lens distortion
  ...
  bool result = cv::solvePnPRansac(p3d,
                                   p2d,
                                   K_cv,
                                   dist_coeffs,
                                   rotationVector,
