...
//    std::cout<<"get test value from Map.h:  "<<slam.getTestValueFromMap()<<std::endl;
  ...
  if (argc < 5) {
    std::cout << "/slam  left_image_directory right_image_directory "
                 "camera_intrinsics_file num_images"
              << std::endl;
    exit(1);
  }


  // TODO read input left_image and right_image.
  ...
  //TODO read camera_instrinsics_path.
  ...
  for (int i = 0; i < num_images; i++) {
    ...
    std::string
        left_image_name = left_image_path + image_name_template + std::to_string(i) + ".png";
    std::string
        right_image_name = right_image_path + image_name_template + std::to_string(i) + ".png";
...
//       cv::waitKey();
    ...
    Sophus::SE3 prePose;
    estimatedPose = slam.estimate3D2DFrontEndWithOpicalFlow(leftImage,
                                                            rightImage,
    ...
    if (!groundTruthPoses.empty() && i < groundTruthPoses.size()) {
      if (i==0) {
        Sophus::SE3 groundTruthPrevPose =
            Sophus::SE3(Eigen::Matrix3d::Identity(), Eigen::Vector3d(0, 0, 0));
        slam.plotTrajectoryNextStep(window,
                                    i,
                                    translGTAccumulated,
